...
export default function RegisterModal() {
    ...
    function canProceedStage1() {
        return (
            email() && username() && !emailExists() && !userNameExists() && checkDateOfBirth()
        );
    }
    ...
    return (
        <Modal
            id="register"
            className={joinClass(" xl:w-[600px] focus:outline-none xl:h-[800px] md:w-[600px] self-center w-full h-full flex flex-col mx-auto xl:mt-12 md:mt-12 md:rounded-xl xl:rounded-xl",
                theme() == "dark" ? "bg-black" : ""
            )}
        >
            <Modal.Content className={joinClass("xl:w-[600px] xl:h-[600px] relative  md:w-[600px] md:h-1/2 w-full h-full", theme() == "dark" ? "bg-black" : "")}>
                    ...
                    <Switch>
                        <Match when={stage() === 1}>
                            <button onClick={() => {
                                ...
                                setPassword("")
                                setPostlyUse("")
                            }} class="  text-lg w-6 h-6 size-6 focus:outline-none  p-2">X</button>
                        </Match>
                        <Match when={stage() === 2}>
                            <button>

                                <ArrowLeft onClick={() => setStage(1)} class="w-6 h-6 size-6   " />
                            </button>
                        </Match>
                    </Switch>
                    <Switch>
