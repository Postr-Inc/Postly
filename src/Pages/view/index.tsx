...
import Carousel, { CarouselItem } from "@/components/UI/UX/Carousel";
import Ellipse from "@/components/Icons/Ellipse";
export default function View(props: any) {
  const { route, params, searchParams, navigate, goBack } = useNavigation(
    "/view/:collection/:id",
  );
    ...
    try {
      ...
      if (api.authStore.model.id !== data?.author?.id) {
        api.metrics.noteMetrics("followed_after_post_view", {
          ...
          hasFollowed: data?.expand.author?.followers?.includes(
  ...
  const LoadingPost = () => (
    <div class="p-4 animate-pulse">
      <div class="flex gap-3">
        <div
          class={`w-10 h-10 rounded-full ${theme() === "dark" ? "bg-gray-700" : "bg-gray-200"}`}
        ></div>
        <div class="flex-1 space-y-3">
          <div class="flex gap-2">
            <div
              class={`h-4 rounded w-20 ${theme() === "dark" ? "bg-gray-700" : "bg-gray-200"}`}
            ></div>
            <div
              class={`h-4 rounded w-16 ${theme() === "dark" ? "bg-gray-700" : "bg-gray-200"}`}
            ></div>
            <div
              class={`h-4 rounded w-12 ${theme() === "dark" ? "bg-gray-700" : "bg-gray-200"}`}
            ></div>
          </div>
          <div class="space-y-2">
            <div
              class={`h-4 rounded w-full ${theme() === "dark" ? "bg-gray-700" : "bg-gray-200"}`}
            ></div>
            <div
              class={`h-4 rounded w-3/4 ${theme() === "dark" ? "bg-gray-700" : "bg-gray-200"}`}
            ></div>
          </div>
          <div class="flex gap-8 mt-4">
            <div
              class={`h-4 rounded w-8 ${theme() === "dark" ? "bg-gray-700" : "bg-gray-200"}`}
            ></div>
            <div
              class={`h-4 rounded w-8 ${theme() === "dark" ? "bg-gray-700" : "bg-gray-200"}`}
            ></div>
  ...
  const CommentWithReplies = (props: {
    ...
    const hasReplies =
      comment.expand?.comments && comment.expand.comments.length > 0;
              ...
              {(reply, replyIndex) => (
                <div
                  ...
                  style={{
                    "margin-bottom":
                      isLast &&
                      replyIndex() === comment.expand.comments.length - 1
                        ? "100px"
                        : "0px",
                  }}
                >
                  ...
                  <div
                    class={joinClass(
                      "absolute left-0 top-4 w-4 h-px ",
                      theme() == "dark"
                        ? "bg-[#141414]"
    ...
    <Page {...{ params, route, navigate: props.navigate }} id={id}>
      ...
      >
        ...
        >
          <div class="flex items-center justify-between px-4 py-3">
            <div class="flex items-center gap-8">
              ...
              <h1
                class={`text-xl font-bold ${theme() === "dark" ? "text-white" : "text-gray-900"}`}
              >
                Post
              </h1>
            </div>
        ...
        <div class="flex-1">
          {/* Post Content */}
          <div
            class={`border-b  ${theme() === "dark" ? "border-[#121212]" : "border-gray-100"}`}
          >
            <Switch
              fallback={
                <div class="p-8 text-center">
                  <div
                    class={`mb-2 ${theme() === "dark" ? "text-red-400" : "text-red-500"}`}
                  >
                    <svg
            ...
            >
              ...
              >
                ...
                <div class="flex flex-col items-start">
                  <span
                    class={`font-medium ${theme() === "dark" ? "text-white" : "text-gray-900"}`}
                  >
                    View Post Analytics
                  </span>
                  <span
          ...
          <div class="pb-20">
            <Switch>
              <Match when={commentsLoading()}>
                <div
                  class={`divide-y ${theme() === "dark" ? "divide-gray-800" : "divide-gray-100"}`}
                >
                  <For each={Array(3).fill(0)}>{() => <LoadingPost />}</For>
                </div>
              </Match>
              <Match
                when={post() && !commentsLoading() && comments().length > 0}
              >
                <div>
                  <For each={comments()}>
                    {(comment, index) => (
              ...
              <Match
                when={post() && !commentsLoading() && comments().length < 1}
              >
                <div class="p-12 text-center">
                  <div class="flex flex-col items-center gap-4">
                    ...
                    <div>
                      <h3
                        class={`text-lg font-medium mb-1 ${theme() === "dark" ? "text-white" : "text-gray-900"}`}
                      >
                        No comments yet
                      </h3>
                      <p
