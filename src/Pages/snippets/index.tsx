...
export default function SnippetReels() {
 ...
 createEffect(() => {
  ...
  createEffect(() => {
    ...
    previousElements.forEach((el) => observer?.unobserve(el));

    // Observe current elements
    const newElements = Array.from(containerRef?.querySelectorAll("[data-index]") || []);
    newElements.forEach((el) => observer?.observe(el));
    previousElements = newElements;
  });
   createEffect(() => {
  const index = activeIndex();
  const isLoaded = videoLoaded()[index]; // only react to this specific boolean

  if (!isLoaded) return;

  videoRefs.forEach((video, i) => {
    if (!video || !videoLoaded()[i]) return;
    if (i === index) {
                  ...
                  >
                    ...
                    <div class="absolute bottom-0 sm:bottom-[120px] left-0 w-full text-white p-4  pointer-events-none z-10">
                      <div class="flex items-center gap-3 mb-2">
                        <Show when={author?.avatar} fallback={<div class="h-10 w-10 rounded bg-gray-600 flex items-center justify-center text-xs">?</div>}>
                          <img
