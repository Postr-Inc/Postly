...
export function SideBarRight(props: {
  ...
  const [searchResults, setSearchResults] = createSignal({ posts: [], users: [] });
  ...
  async function search() {
    ...
    try {
      const results = (await api.deepSearch(["users", "posts"], searchQuery())) as any;
        ...
        <ul class="p-2 w-80 min-h-full gap-5 flex flex-col text-base-content">
          ...
          <li ref={(el) => (containerRef = el)} class="relative w-full max-w-lg">
            ...
            <Show when={showResults()}>
              <div class="absolute z-50 w-full bg-base-100 border border-base-300 shadow-lg rounded-xl mt-2 p-4 space-y-2 max-h-[400px] overflow-auto">
                ...
                {/* 🔍 Recent Searches */}
                <Show when={!searching() && searchQuery().trim() === "" && recentSearches().length > 0}>
                  <div>
                ...
                <Show when={!searching() && searchResults().users.length === 0 && hasSearched()}>
          ...
          <Switch>
            <Match when={!RelevantPeople() || RelevantPeople().length === 0}>
              <li class={`${theme() === "dark" ? "xl:border xl:border-[#121212]" : "xl:border xl:border-[#d3d3d3d8]"} p-5 rounded-xl`}>
                    ...
                    {(item) => (
                      <div class="flex flex-row gap-5">
                        <Switch>
                          ...
                          <Match when={item.avatar}>
                            <img class="w-10 h-10 rounded" src={api.cdn.getUrl("users", item.id, item.avatar)} alt={item.username} />
                          </Match>
                        </Switch>
                        <div
          ...
          <a href="https://www.producthunt.com/products/postly-5?embed=true&utm_source=badge-featured&utm_medium=badge&utm_source=badge-postly&#0045;7" target="_blank">
            <img
          ...
          <div class="flex flex-col gap-5 mt-2 p-2 text-sm">
            <li class="flex flex-row gap-5">
              <a href="/information/terms.pdf" target="_blank" class="cursor-pointer hover:underline">
                Terms of service
              </a>
              <a href="/information/privacy.pdf" target="_blank" class="cursor-pointer hover:underline">
                Privacy Policy
              </a>
            </li>
            <li class="flex flex-row gap-5">
              <a onclick={() => navigate("/safety")} class="cursor-pointer hover:underline">
                Help and safety
              </a>
            ...
            <li class="flex flex-row gap-5">
              ...
              <div class="tooltip cursor-pointer" data-tip="View Service Statuses">
                <a class="cursor-pointer hover:underline" onClick={() => navigate("/status")}>Status Page</a>
              </div>
            </li>
            <li>
              <a>© 2022 - {new Date().getFullYear()} Pascal. All rights reserved</a>
            </li>
          </div>
        </ul>
      </div>
